...
def convert(h, m, s):
    seconds = (h * 3600) + (m * 60) + s                       # to covert the input time to seconds
    return seconds
...
def reset():
    ...
    reset_fn = True
    remtime = 0
    timeformat = datetime_time(00, 00, 00)                    # to reset time back to 00:00:00
...
for i in range(6):                                            # Configure the grid layout
    root.columnconfigure(i, weight=1)
for i in range(5):
    root.rowconfigure(i, weight=1)


try:                                                          # Load the hourglass gif and its frames
...
def update_hourglass():                                                         #to start and move the hourglass animation
...
label = tk.Label(root, text="SET TIMER", fg="grey14" , font=("Courier", 24, "bold"), bg="ghost white")  # Large bold title
label.grid(row=0, column=2, columnspan=2, pady=10)

hour_label = tk.Label(root, text="Hour:", font=("Arial", 12), bg="ghost white") # Spinbox and labels for hours, minutes, and seconds
hour_label.grid(row=1, column=0, padx=(10, 0), pady=10, sticky="e")
...
# Timer display
display_time = tk.Label(root, text="00:00:00", fg="white", font=("Courier", 24, "bold"), width=8, bg="grey14")  # digital-style font for timer
...
button = tk.Button(root, text="Start", command=funct, bg="RoyalBlue2", fg="ghost white", font=("Arial", 12), width=10)  # Blue Start button
button.grid(row=3, column=2, columnspan=2, pady=10)

stopbutton = tk.Button(root, text="Pause", command=pause, bg="orange", fg="ghost white", font=("Arial", 12), width=10)  # Orange Pause button
stopbutton.grid(row=4, column=1, columnspan=2, pady=10)

resumebutton = tk.Button(root, text="Resume", command=resume, bg="green", fg="ghost white", font=("Arial", 12), width=10)  # Green Resume button
resumebutton.grid(row=4, column=1, columnspan=2, pady=10)

resetbutton = tk.Button(root, text="Reset", command=reset, bg="red", fg="ghost white", font=("Arial", 12), width=10)  # Red Reset button
resetbutton.grid(row=4, column=3, columnspan=2, pady=10)

remove_stopbutton_buttons()                                                     # Initial button states
remove_resume_buttons()
remove_reset_buttons()
